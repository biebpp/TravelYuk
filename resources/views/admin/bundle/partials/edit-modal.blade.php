<x-modal name="edit-bundle-modal" focusable>
    <form method="POST" :action="`/admin/bundles/${selectedBundle.id}`" class="p-6">
        ...
        <div class="mt-4 space-y-4">
            <!-- Name -->
            <div>
                <x-input-label for="edit_name" :value="__('Bundle Name')" />
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <x-input-label for="edit_price" :value="__('Price ($)')" />
                    <x-text-input id="edit_price" name="price" type="number" step="0.01" class="mt-1 block w-full"
                ...
                <div>
                    <x-input-label for="edit_slot" :value="__('Available Slots')" />
                    <x-text-input id="edit_slot" name="slot" type="number" class="mt-1 block w-full"
            ...
            <div x-data="{ 
                addDestination() {
                    if (!selectedBundle.destination_ids) {
                        selectedBundle.destination_ids = [];
                    }
                    selectedBundle.destination_ids.push('');
                },
                removeDestination(index) {
                    if (selectedBundle.destination_ids.length > 1) {
                        selectedBundle.destination_ids.splice(index, 1);
                    }
                }
            }">
                ...
                <div class="space-y-2">
                    <template x-for="(destId, index) in selectedBundle.destination_ids" :key="index">
                        <div class="flex items-center gap-2">
                            <select :name="`destination_ids[${index}]`" x-model="selectedBundle.destination_ids[index]"
                                class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                <option value="" disabled>{{ __('Select a Destination') }}</option>
                            ...
                            <button type="button" x-on:click="removeDestination(index)" 
                                x-show="selectedBundle.destination_ids.length > 1"
